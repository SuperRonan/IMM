...
int main()
{
	...
	for(int i=0; i<h; ++i)
	{
		for(int j=0; j<w; ++j)
		{
			DesiredFeatures[i * w + j] = ImageD[i + Hmin][j + Wmin];
		}
	}
    ...
    Lspi = Ls.pseudoInverse();
    //std::cout<<Ls<<std::endl;
    //std::cout<<Lspi<<std::endl;
	...
	vpColVector VisualFeatureError, VisualFeatureErrorSum(h * w);
    ///////////////////////////////////////////////////////////////////////
    //
    // Control loop
    //
    ...
    try  {
        while(1)
        {
            ...
            for(int i=0; i<h; ++i)
			{
				for(int j=0; j<w; ++j)
				{
					CurrentFeatures[i * w + j] = Image[i + Hmin][j + Wmin];
				}
			}
            ...
            //
             cost = (VisualFeatureError/nbPx).euclideanNorm();
            ...
            //
            object_velocity[0] = t; // translation (translational velocity has to be provided in meter/s)
            object_velocity[1] = t; // translation (translational velocity has to be provided in meter/s)
            object_velocity[2] = t; // translation (translational velocity has to be provided in meter/s)
            object_velocity[3] = r; // rotation (angular velocity has to be provided in rad/s. Use vpMath::rad() for converting degree to rad)
            object_velocity[4] = r; // rotation (angular velocity has to be provided in rad/s. Use vpMath::rad() for converting degree to rad)
            object_velocity[5] = r; // rotation (angular velocity has to be provided in rad/s. Use vpMath::rad() for converting degree to rad)
#endif
